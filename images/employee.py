...
class employeeClass:
    def __init__(self,root):
         ...
         #===========search frame=====
         SearchFrame=LabelFrame(self.root,text="Search Employe",font=("goudy old style",12,"bold"),bd=2,bg="white")
         ...
         txt_search=Entry(SearchFrame,textvariable=self.var_seachtxt,font=("goudy old style",15),bg="lightyellow").place(x=200,y=10)
        ...
        #============row3==========
         ...
         #============buttons====
         btn_add=Button(self.root,text="Save",command=self.add,font=("goudy old style",15),bg="sky blue",fg="white",cursor="hand2").place(x=500,y=305,width=110,height=28)
         btn_update=Button(self.root,text="Update",command=self.update,font=("goudy old style",15),bg="green",fg="white",cursor="hand2").place(x=620,y=305,width=110,height=28)
         ...
         btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="grey",fg="white",cursor="hand2").place(x=860,y=305,width=110,height=28)
...
#=====================================================================================
    ...
    def show(self):
        ...
        try:
            ...
            for row in rows:
                self.EmployeeTable.insert('',END,value=row)
    ...
    def search(self):
        ...
        try:
            ...
            elif self.var_seachtxt.get()=="":
                messagebox.showerror("Error","Search input should be required",parent=self.root)
            else:    
              cur.execute("select * from employee where "+self.var_searchby.get()+" LIKE '%"+self.var_seachtxt.get()+"%'")
              rows=cur.fetchall()
              if len(rows)!=0:
               self.EmployeeTable.delete(*self.EmployeeTable.get_children())
               for row in rows:
