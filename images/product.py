...
class productClass:
    def __init__(self,root):
         ...
         #============buttons====
         btn_add=Button(product_Frame,text="Save",command=self.add,font=("goudy old style",15),bg="sky blue",fg="white",cursor="hand2").place(x=10,y=400,width=100,height=40)
         ...
         self.product_table.heading("Category",text="Catergory")
    ...
    def update(self):
        ...
        try:
            if self.var_pid.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Prroduct Id",parent=self.root)
                else:
        ...
        try:
            if self.var_searchby.get()=="Select":
             messagebox.showerror("Error","Select search by option",parent=self.root)
              ...
              cur.execute("select * from product where "+self.var_searchby.get()+" LIKE '%"+self.var_seachtxt.get()+"%'")
              rows=cur.fetchall()
              if len(rows)!=0:
               self.product_table.delete(*self.product_table.get_children())
               for row in rows:
